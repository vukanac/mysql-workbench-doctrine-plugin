...
-- CHANGELOG:
-- 0.1alpha7
--    + changed the license from GPLv2 to LGPLv3
--    + removed plural correction of table names (deprecated in doctrine 1.0)
-- 0.1alpha6
...
-- 0.1alpha4
--    + tables_has_names -> TablesName ->[fix]-> TableName
--    + [improvement] rename columns "idtable | table_idtable" -> "id | table_id"
-- 0.1alpha3
--    + [improvement] convert underscores in tablenames to CamelCase
-- 0.1alpha2
--    + nestedset support for tablenames ending with _ns
-- 0.1alpha1
--    supports:
--    + indexes [fulltext, unique, index]
--    + index length
--    + collation
--    + character set
--    + engine [MySQL, InnoDB]
...
--    + table name fixing
--    + column flags [binary, zerofill, unsigned]
--    + autoincrement
--    + not null
--    + default values
--    + decimal precision
--    + column length [e.g. varchar(255)]
--
---------------------------------------------------------------------------------------------
...
function exportYamlSchemaToClipboard(cat)
